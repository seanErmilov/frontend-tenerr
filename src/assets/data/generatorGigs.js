...
const imgs = formattedGigs.map(gig => gig.imgUrl)

// console.log(getRandomElements(imgs, 4));
// const imgs = formattedGigs.map(gig => gig.imgUrl)



const levels = ['basic', 'premium', 'standard']

var mappedGigs = formattedGigs.map(gig => ({ ...gig, imgUrl: getRandomElements(imgs, 4) }))
mappedGigs = mappedGigs.map(gig => ({...gig, reviews: gig.reviews.map(rev => ({...rev, 'rate': getRandomIntInclusive(3, 5)}))}))


mappedGigs = mappedGigs.map(gig => ({...gig, owner: {...gig.owner, level: getRandomElements(levels, 1)}}))
// console.log(mappedGigs[0].owner.level)
// console.log()


console.log(JSON.stringify(mappedGigs));